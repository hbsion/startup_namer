...
class PositionWidget extends StatelessWidget {
  ...
  const PositionWidget({Key key, @required this.outcomeIds, @required this.eventId}) : super(key: key);
  ...
  Widget _buildHeader(BuildContext context, _ViewModel model) {
    List<Widget> widgets = [];
    widgets.add(Expanded(flex: 3, child: new Text("Each way terms: ${model.betOffers[0].eachWay?.terms ?? ""}")));
